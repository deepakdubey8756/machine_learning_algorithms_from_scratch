...
#implementing Vectorized Linear Regresssion from scratch using numpy
...
class LinearReression:
    ...
    def predict(self, x):
        """X: numpy array to features
           Theta: numpy array of weights"""
        y_predicted = np.dot(self.theta, x) + self.bias
        return y_predicted


    # this function will return initial coeffients
    def initialize_coeff(self, features):        
        return np.linspace(0, 0, features)
...
if __name__ == "__main__":
    ...
    print(leg.theta)
    print(leg.bias)
    print(leg.predict(np.array([45, 90])))
    print(leg.mean_squared_error(y,  x))
